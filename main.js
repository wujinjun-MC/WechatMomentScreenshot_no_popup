...
function getArticleInfo() {
    ...
    mdui.prompt("公众号文章链接", (function(e) {
        var n = new XMLHttpRequest;
        n.open("GET", "https://i.akarin.dev/wmsproxy/?url=" + encodeURIComponent(e)),
        n.onreadystatechange = function() {
            if (4 == n.readyState && 200 == n.status) {
                var e = JSON.parse(n.responseText);
                document.getElementById("requestAction").innerText = "OK",
                "opening" != t.getState() && "opened" != t.getState() || (void 0 !== e && e.success ? (document.getElementById("configArticleTitle").value = e.title,
                document.getElementById("articleTitle").innerText = e.title,
                document.getElementById("articleIcon").style.backgroundImage = 'url("' + e.cover + '")',
                document.getElementById("requestResult").innerText = "获取成功！(ゝ∀･)",
                document.getElementById("requestResultContent").innerHTML = "标题：" + e.title + '<br>封面：<a target="_blank" href="' + e.cover + '">点此查看</a>') : (document.getElementById("requestResult").innerText = "获取失败！( ´_っ`)",
